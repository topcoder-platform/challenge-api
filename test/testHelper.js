...
async function createData() {
  ...
  completedChallenge = await helper.create(
    "Challenge",
    _.assign(challengeData, { id: uuid(), status: constants.challengeStatuses.Completed })
  );
  ...
  await esClient.create({
    ...
    body: _.assignIn(
      { numOfSubmissions: 0, numOfRegistrants: 0 },
      completedChallenge.originalItem()
    ),
    refresh: "true", // refresh ES so that it is visible for read operations instantly
  });
}
